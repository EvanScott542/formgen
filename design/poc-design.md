...
Prompt to user:
  ...
  3. Nx version / preset? (latest recommended)
  4. Node version? (20.x or 22.x)
  5. Where should all projects live? (e.g., ~/projects/formgen-poc/)
  6. Python version for FastAPI? (3.11+ recommended)
  7. Postgres: Docker Compose or native install?
```

### Block 2: Configurator Monorepo

```bash
cd <base_dir>
npx create-nx-workspace@latest <configurator-workspace-name> \
...
cd <configurator-workspace-name>
npx nx g @angular/material:ng-add --project=<configurator-app-name>
```

### Block 3: Formgen Library (standalone)

```bash
cd <base_dir>
mkdir <formgen-lib-dir> && cd <formgen-lib-dir>
ng new <formgen-lib-name> --create-application=false
cd <formgen-lib-name>
ng generate library <formgen-lib-name>
# Add Angular Material as peer dependency, configure ng-packagr
```

### Block 4: Playground Monorepo

```bash
cd <base_dir>
npx create-nx-workspace@latest <playground-workspace-name> \
  --preset=angular-monorepo \
...
cd <playground-workspace-name>
npx nx g @angular/material:ng-add --project=<playground-app-name>
# Link formgen library (npm link or file: reference)
```
...
# Create: main.py, shared/database.py, shared/renderable_form.py
# Create per-context: router.py, service.py, schemas.py, models.py, translator.py
# .env with DATABASE_URL and LLM API key
```
...
docker compose up -d
cd <configurator-workspace-name>/apps/api
...
# Terminal 3: Configurator Angular
cd <configurator-workspace-name>
npx nx serve <configurator-app-name>

# Terminal 4: Playground Angular
cd <playground-workspace-name>
npx nx serve <playground-app-name> --port 4201

# Verify:
# - http://localhost:4200 → Configurator shell renders
# - http://localhost:4201 → Playground shell renders
# - http://localhost:8000/docs → FastAPI Swagger — four route groups visible
# - formgen library builds: cd <formgen-lib-dir> && ng build <formgen-lib-name>
```
